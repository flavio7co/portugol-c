...
void tabela()
{
	...
	strcpy(tag[9].p,"leia(");
	strcpy(tag[9].c,"scanf(tipo(sizeof(");

	strcpy(tag[10].p,"leia (");
	strcpy(tag[10].c,"scanf (tipo(sizeof(");
	
	strcpy(tag[11].p,"escreva(");
	strcpy(tag[11].c,"printf(tipo(sizeof(");

	strcpy(tag[12].p,"escreva (");
	strcpy(tag[12].c,"printf (tipo(sizeof(");
		...
		{ 
				...
				if(strlen(tag[n].p)==ok && ok!=0)
				{
					...
					for(x-=ok-1;cont<strlen(tag[n].c);x++)
					{
  				    ...
  				    if(n==9)strcpy(tag[n].c, "scanf(tipo(sizeof(");
  				    if(n==10)strcpy(tag[n].c, "scanf (tipo(sizeof(");
  				    if(n==11)strcpy(tag[n].c, "printf(tipo(sizeof(");
  				    if(n==12)strcpy(tag[n].c, "printf (tipo(sizeof(");
				}
				//linha2[x]=linha[x];
		}
	}
...
void criaprincipal()
{
	...
	if(arqprinc == NULL){
	...
	} else
	  {
			...
			if (result == EOF) printf("Erro na Gravacao\n");
			result = fputs("typedef double real;\n", arqprinc);
			if (result == EOF) printf("Erro na Gravacao\n");
			result = fputs("typedef char literal;\n", arqprinc);
			if (result == EOF) printf("Erro na Gravacao\n");
			result = fputs("typedef char caracter;\n", arqprinc);
			if (result == EOF) printf("Erro na Gravacao\n");
			result = fputs("char* tipo(int x){switch(x){case 1: return \" %c\";case 4: return \"%d\";case 8:  return \"%lf\";default: return \"%s\";}}\n", arqprinc);
			if (result == EOF) printf("Erro na Gravacao\n");
...
void criafuncao()
{
	...
	while (!feof(arqin))
	{
      res = fgets(linha, 200, arqin);  // o 'fgets' lê até 99 caracteres ou até o '\n'
      if (res)
      {  
		traduzlinha();
	...
	while (!feof(arqin))
	{
      ...
      {  
		...
		{ 
				...
				if(linha[x]=='=')
				  if(linha[x+1]=='=' || linha[x-1]=='<' || linha[x-1]=='>' || linha[x-1]=='!') x+=1;
				    else return 0;
		}
	  }
	}
		  ...
		  else
		  {
			...
			if(arqfunc == NULL){
			...
			} else
	          {
			      ...
			      result = fscanf(arqin, "%s %s",agrt,nomeprog); //(Linha, 200, arqin);  // o 'fgets' lê até 99 caracteres ou até o '\n'
			      printf("Programa a ser compilado: %s ... Executando:\n", nomeprog);
			      res = fgets(linha, 200, arqin);
